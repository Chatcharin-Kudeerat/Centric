...
class WebsocketSession {
  ...
  _sendS(audioFormat, callStartTime, callID1, agentID) {
    return new Promise(async (resolve, reject) => {
      try {
        if (this.status != WebsocketSession.STATUS.OPENED) {
          throw new Error(`Unexpected _sendS() ${this.status}`)
        }
        this.socket.send(`${WebsocketSession.COMMAND.S} ${audioFormat} ${JSON.stringify({callStartTime, callID1, agentID})}`);
...
class EsasClientV1 {
  ...
  startWebsocketSession(audioFormat, callStartTime, callID1, agentID, callbacks) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        socket.onopen = async (e) => {
          try {
            ...
            await websocketSession._sendS(audioFormat, callStartTime, callID1, agentID);

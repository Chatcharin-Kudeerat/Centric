...
describe('v1', ()=> {
  ...
  describe('RTP', ()=> {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          await Promise.all([
            ...
            lib.cmd(`${config.ffmpeg} -re -i ../samples/10sec_1ch_8000khz_16bit.wav -c:a pcm_s16le -f rtp 'rtp://127.0.0.1:35001'`),
    ...
    it('ESASQA result 8kHz Port0', () => {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          expect(results[0].Param[0].Segment).to.equals(1);
          expect(results[0].Param[0].Channel).to.equals(0);
          expect(results[1].Param[0].Segment).to.equals(2);
          expect(results[1].Param[0].Channel).to.equals(0);
          expect(results[2].Param[0].Segment).to.equals(3);
          expect(results[2].Param[0].Channel).to.equals(0);
          expect(results[3].Param[0].Segment).to.equals(4);
          expect(results[3].Param[0].Channel).to.equals(0);
          expect(results[4].Param[0].Segment).to.equals(5);
          expect(results[4].Param[0].Channel).to.equals(0);
          results[0].Param = [];
    ...
    it('ESASQA result 8kHz Port1', () => {
      return new Promise(async (resolve, reject) => {
        try {
          const res = await axios.get(`http://localhost/ESASQA/${this.sessionId8kHz}/35001`, {}, {});
          const results = _.chain(res.data.split('\n'))
                .map((line) => {
                  try { return JSON.parse(line) } catch (e) { return null};
                })
          ...
          expect(_.keys(results[0].Param[0])).to.deep.equals(PARAM_KEYS);
          expect(results[0].Param[0].Segment).to.equals(1);
          expect(results[0].Param[0].Channel).to.equals(1);
          expect(results[1].Param[0].Segment).to.equals(2);
          expect(results[1].Param[0].Channel).to.equals(1);
          expect(results[2].Param[0].Segment).to.equals(3);
          expect(results[2].Param[0].Channel).to.equals(1);
          expect(results[3].Param[0].Segment).to.equals(4);
          expect(results[3].Param[0].Channel).to.equals(1);
          expect(results[4].Param[0].Segment).to.equals(5);
          expect(results[4].Param[0].Channel).to.equals(1);
          results[0].Param = [];
          expect(results[0]).to.deep.equals({
            callID: 'callID_0001',
            AgentID: 'agent_0001',
  ...
  describe('Websocket', ()=> {
    const startWebsocketSession = (type, callId, audioFormat, byteRate, file) => {
            ...
            const interval = setInterval(async () => {
              ...
              try {
                if (!chunk) {
                  ...
                  expect(results.length).to.equals(5);
                  expect(_.keys(results[0].Param[0])).to.deep.equals(PARAM_KEYS);
                  expect(results[0].Param[0].Channel).to.equals((type == 'operator' ? 0 : 1));
                  results[0].Param = [];
                  expect(results[0]).to.deep.equals({
                    callID: callId,
                    AgentID: 'agent_0002',
    ...
    startWebsocketSession('operator', 'callID_0011', 'lsb8k', 16000, '../samples/10sec_1ch_8000khz_16bit_le.raw');
    startWebsocketSession('customer', 'callID_0012', 'msb8k', 16000, '../samples/10sec_1ch_8000khz_16bit_be.raw');
    startWebsocketSession('operator', 'callID_0013', 'lsb16k', 32000, '../samples/10sec_1ch_16000khz_16bit_le.raw');
    startWebsocketSession('customer', 'callID_0014', 'msb16k', 32000, '../samples/10sec_1ch_16000khz_16bit_be.raw');
  });
});
